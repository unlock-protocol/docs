---
title: All Features
description:
  a three-wide set of cards edited and stuff
sidebar_position: 5
---
<div className="container getting-started">
  <div className="row">
    <div className="col col--12">
      <h3>
        Unlock is a protocol for creating memberships and subscriptions as NFTs
      </h3>
      <p>
        Explore our developer documentation including a complete tutorial in order to integrate Unlock into your website or application.
      </p>
    </div>
  </div>
  <div className="row">
    <div className="col">
      <div className="row">
        <div className="card col--4">
          <div className="card__header">
            <h3>Feature 1</h3>
          </div>
          <div className="card__body">
            Feature description.
          </div>
        </div>
        <div className="card col--4">
          <div className="card__header">
            <h3>Feature 2</h3>
          </div>
          <div className="card__body">
            Feature description.
          </div>
        </div> 
        <div className="card col--4">
          <div className="card__header">
            <h3>Feature 3</h3>
          </div>
          <div className="card__body">
            Feature description.
          </div>
        </div>
      </div>
      <div className="row">
        <div className="card">
          <div className="card__header">
            <h3>Feature name</h3>
          </div>
          <div className="card__body">
            feature description
          </div>
        </div>
      </div>
      <div className="row">
        <div className="card">
          <div className="card__header">
            <h3>Feature name 2</h3>
          </div>
          <div className="card__body">
            Feature description.
          </div>
        </div>
      </div>
      <div className="row">
        <div className="card">
          <div className="card__header">
            <h3>Feature name 3</h3>
          </div>
          <div className="card__body">
            Feature description.
          </div>
        </div>
      </div>
    </div>
    <div className="col col--4">
      <div className="row">
        <div className="card">
          <div className="card__header">
            <h3>Feature name 4</h3>
          </div>
          <div className="card__body">
            Feature description.
          </div>
        </div>
      </div>
      <div className="row">
        <div className="card">
          <div className="card__header">
            <h3>Feature name 5</h3>
          </div>
          <div className="card__body">
            Feature description.
          </div>
        </div>
      </div>
      <div className="row">
        <div className="card">
          <div className="card__header">
            <h3>Feature name 6</h3>
          </div>
          <div className="card__body">
            Feature description.
          </div>
        </div>
      </div>
      <div className="row">
        <div className="card">
          <div className="card__header">
            <h3>New to web3?</h3>
          </div>
          <div className="card__body">
            Gain an understanding of key concepts unique to web3 but not
            specific to Unlock Protocol.
          </div>
        </div>
      </div>
    </div>
    <div className="col col--4">
      <div className="row">
        <div className="card">
          <div className="card__header">
            <h3>Why Unlock?</h3>
          </div>
          <div className="card__body">
            Details about how Unlock is built and the specific use cases it
            enables.
          </div>
        </div>
      </div>
      <div className="row">
        <div className="card">
          <div className="card__header">
            <h3>Developer Tools</h3>
          </div>
          <div className="card__body">
            Get up and running with libraries, APIs and integration tools.
          </div>
        </div>
      </div>
      <div className="row">
        <div className="card">
          <div className="card__header">
            <h3>Examples & Tutorials</h3>
          </div>
          <div className="card__body">
            Learn how to interact with the smart contracts or use our template
            repos.
          </div>
        </div>
      </div>
      <div className="row">
        <div className="card">
          <div className="card__header">
            <h3>New to web3?</h3>
          </div>
          <div className="card__body">
            Gain an understanding of key concepts unique to web3 but not
            specific to Unlock Protocol.
          </div>
        </div>
      </div>
    </div>
    <div className="col col--4">
      <div className="row">
        <div className="card">
          <div class="card__header">
            <h3>Glossary of Terms</h3>
          </div>
          <div className="card__body">
            In the Unlock ecosystem...
            <dl>
              <dt>Membership</dt>
              <dd>
                A connection between a creator, brand or project and a fan or
                community member. A membership usual confers benefits or special
                access to those who hold it. Members can also connect directly
                with each other.
              </dd>
              <dt>Lock</dt>
              <dd>
                a “Lock” is a smart contract that creates (or “mints”) NFTs.
                Locks also can be used to ensure “members-only” access to
                resources like online content, live events, online
                certifications, or other “members-only” experiences.
              </dd>
              <dt>Key</dt>
              <dd>
                a “Key” is an NFT (minted by a Lock) that has additional
                features added to it to facilitate the management of
                memberships, such as an expiration date.
              </dd>
              <dt>Dashboard</dt>
              <dd>
                A web app built by Unlock Labs for creating "Locks", managing
                "Keys", and more.
              </dd>
            </dl>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>

<div class="container">
  <div class="full-width-section">
    <h2>Foo</h2>
    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla convallis consequat nisl, non commodo turpis gravida eget.</p>
  </div>
  
  <div class="row">
    <div class="col">
      <div class="feature">
        Content for feature 1
      </div>
    </div>
    <div class="col">
      <div class="feature">
        Content for feature 2
      </div>
    </div>
    <div class="col">
      <div class="feature">
        Content for feature 3
      </div>
    </div>
  </div>
  
  <div class="row">
    <div class="col">
      <div class="feature">
        Content for feature 4
      </div>
    </div>
    <div class="col">
      <div class="feature">
        Content for feature 5
      </div>
    </div>
    <div class="col">
      <div class="feature">
        Content for feature 6
      </div>
    </div>
  </div>
  
  <div class="full-width-section">
    <h2>Bar</h2>
    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla convallis consequat nisl, non commodo turpis gravida eget.</p>
  </div>
  
  <div class="row">
    <div class="col">
      <div class="feature">
        Content for feature 7
      </div>
    </div>
    <div class="col">
      <div class="feature">
        Content for feature 8
      </div>
    </div>
    <div class="col">
      <div class="feature">
        Content for feature 9
      </div>
    </div>
  </div>
</div>

<div class="container">
  <div class="full-width-section">
    <h2>Membership smart contracts</h2>
    <div class="row">
      <div class="col">
        <div class="feature">
          Deploy membership smart contracts
        </div>
      </div>
      <div class="col">
        <div class="feature">
          Custom contract name
        </div>
      </div>
      <div class="col">
        <div class="feature">
          Custom ticker symbol for membership tokens
        </div>
      </div>
    </div>
    <div class="row">
      <div class="col">
        <div class="feature">
          Upgradable smart contracts
        </div>
      </div>
    </div>
  </div>
  
  <div class="full-width-section">
    <h2>Membership management</h2>
    <div class="row">
      <div class="col">
        <div className="card">
        <div class="feature" className="card__header">
          Time-based memberships (with customizable membership duration from 1 second to 10000000+ years)
        </div>
        </div>
    </div>
      <div class="col">
        <div className="card">
        <div class="feature" className="card__body">
          Non-expiring memberships
        </div>
        </div>
      </div>
      <div class="col">
        <div className="card">
        <div class="feature" className="card__body">
          Set membership duration
        </div>
        </div>
      </div>
    </div>
    <div class="row">
      <div class="col">
        <div className="card">
        <div class="feature" className="card__header">
          Set membership price
        </div>
        </div>
      </div>
      <div class="col">
        <div className="card" className="card__header">
        <div class="feature">
          Set membership quantity
        </div>
        </div>
      </div>
      <div class="col">
        <div className="card" className="card__header">
        <div class="feature">
          Set maximum memberships per holder
        </div>
        </div>
      </div>
    </div>
    <div class="row">
      <div class="col">
        <div class="feature">
          Free memberships
        </div>
      </div>
      <div class="col">
        <div class="feature">
          Claimable memberships
        </div>
      </div>
      <div class="col">
        <div class="feature">
          Cancel memberships
        </div>
      </div>
    </div>
    <div class="row">
      <div class="col">
        <div class="feature">
          Extend memberships
        </div>
      </div>
      <div class="col">
        <div class="feature">
          Membership renewals
        </div>
      </div>
      <div class="col">
        <div class="feature">
          Member lists
        </div>
      </div>
    </div>
    <div class="row">
      <div class="col">
        <div class="feature">
          Membership status indicator (valid, expired)
        </div>
      </div>
      <div class="col">
        <div class="feature">
          Renew memberships for others
        </div>
      </div>
      <div class="col">
        <div class="feature">
          Automated membership renewals
        </div>
      </div>
    </div>
  </div>
  
  <div class="full-width-section">
    <h2>Subscriptions</h2>
    <div class="row">
      <div class="col">
        <div class="feature">
          Recurring subscriptions onchain
        </div>
      </div>
      <div class="col">
        <div class="feature">
          Pay for subscriptions via credit cards
        </div>
      </div>
      <div class="col">
        <div class="feature">
          Pay for subscriptions via stablecoins
        </div>
      </div>
    </div>
    <div class="row">
      <div class="col">
        <div class="feature">
          Pay for subscriptions via native tokens
        </div>
      </div>
      <div class="col">
        <div class="feature">
          Pay for subscriptions via ERC-20 tokens
        </div>
      </div>
    </div>
  </div>
</div>
